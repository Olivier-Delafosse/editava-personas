---
name: editorial-feed
description: "Fetch verified editorial knowledge — recent AI/ML/tech/crypto news, papers, products, incidents, with primary sources and dates — from CoreProse Editorial Intelligence. Use when a persona needs to ground a factual claim in a verifiable source, cite a paper or product release, or check 'compared to what / on what evidence' against real data. Returns: structured JSON with title, summary, source label, source URL, published date, semantic relevance score. Required when the persona's reply will assert dates, numbers, or named events."
license: MIT
...
metadata:
  author: editava
  version: "1.0.0"
  repository: "https://github.com/Olivier-Delafosse/editava-personas"
  ...
  upstream: "CoreProse Editorial Intelligence (Elasticsearch-backed)"
---
...
Headers (optional):
- `x-api-key: <user's CoreProse key>` — if set, raises the tier from `anonymous` (5/day, 3 results) to whatever the user's plan grants. Without the header, the request still works at the anonymous tier.
...
curl -s "https://www.coreprose.com/api/public/kb-feed?niche=ia&q=$(printf %s 'mistral release' | jq -sRr @uri)&country=US&limit=5" \
  ${COREPROSE_API_KEY:+-H "x-api-key: $COREPROSE_API_KEY"}
```
...
| HTTP 429 rate limit | (future, not enforced yet) | Persona acknowledges the limit in voice; suggests user set `COREPROSE_API_KEY` for higher tier |
...
- **Free tier** (50/day, 10 results): sign up at https://www.coreprose.com/signup
- **Pro tier** (1k/day, 25 results, pro index): https://www.coreprose.com/pricing
- **Business tier** (unlimited, 50 results, all indices): contact sales
...
The persona runtime should pick it up from `process.env.COREPROSE_API_KEY` and pass it as the `x-api-key` header.
